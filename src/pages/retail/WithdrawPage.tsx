...
export default function WithdrawPage() {
    ...
    const handleRequestOtp = async () => {
        ...
        try {
            ...
            setVerifyError(err.response?.data?.detail || 'Failed to send verification code. Please try again.');
        } finally {
            setSendingOtp(false);
        }
    };
                ...
                {verifyStep === 'review' ? (
                    ...
                    <>
                        <div className="text-center mb-6">
                            ...
                            <p className="text-sm text-gray-400 mt-1">A verification code has been sent to your email.</p>
                        </div>

                        <div className="space-y-4 mb-6">
                            <div>
                                <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2">Email verification code</label>
                                <input
                                    type="text"
                                    inputMode="numeric"
                                    autoFocus
                                    ...
                                    className="w-full bg-[#0B0E14] border border-[#1E2533] focus:border-emerald-500/50 outline-none rounded-xl py-3.5 px-4 text-lg tracking-widest text-white font-mono text-center"
                                />
                            </div>

                            {totpEnabled && (
                                <div>
                                    <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2">Authenticator app code</label>
                                    <input
                                        type="text"
                                        inputMode="numeric"
                                        value={totpCode}
                            ...
                            <button type="button" onClick={handleRequestOtp} disabled={sendingOtp} className="text-xs font-bold text-blue-400 hover:text-blue-300 disabled:opacity-50">
    ...
    return (
        <div className="max-w-[1200px] mx-auto space-y-6 animate-in slide-in-from-right-8 duration-300 pt-4 px-4 md:px-0">
            ...
            <div className="space-y-3">
                {!totpEnabled && (
                    <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-300 text-sm font-medium flex items-center gap-3 animate-in slide-in-from-top-2 duration-300">
                                            ...
                                            {(ASSET_NETWORKS[cryptoAsset] || []).map(net => (
                                                <button
                                                    ...
                                                    className={`p-4 rounded-xl text-left transition-all duration-200 border relative overflow-hidden group
                                                        ${!net.active
                                                            ? 'bg-[#0B0E14]/30 border-[#1E2533]/30 opacity-50 cursor-not-allowed'
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-start justify-between mb-3">
                                                        <span className={`text-sm font-bold transition-colors ${!net.active ? 'text-gray-500' : cryptoNetwork === net.id ? 'text-white' : 'text-gray-300'}`}>
                                                            {net.name}
                                                        </span>
                                                        <div className="flex items-center gap-1.5">
